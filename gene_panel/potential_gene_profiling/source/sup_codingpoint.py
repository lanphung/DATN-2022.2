...
s = pd.read_csv(dp['msample'], encoding = 'latin-1',usecols = ['COSMIC_SAMPLE_ID','ETHNICITY','IS_ASIAN','ID_INDIVIDUAL'], dtype = str, low_memory = False)
...
for chunk in chunks:
    ...
    _chunk += 1
    if _chunk == 1: 
        results.to_csv(dp['sup_codmut'], encoding = 'latin-1', mode = 'w', index = False)
        results = pd.DataFrame(columns = hdr.columns, dtype = str)    
    else : 
        results.to_csv(dp['sup_codmut'], encoding = 'latin-1', mode = 'a', header = False, index = False)
        results = pd.DataFrame(columns = hdr.columns, dtype = str)
