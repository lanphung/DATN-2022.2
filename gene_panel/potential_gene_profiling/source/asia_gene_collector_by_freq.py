...
data = pd.read_csv(dp['sup_acodmut'], encoding = 'latin-1', dtype = str, usecols = ['GENE_SYMBOL', 'ID_INDIVIDUAL','CLASS'])
...
#    result.to_csv(f'/media/data/thanhnb/COSMIC_individual_/out/freq/{c}.csv', index = False)
    ...
    print(f"----- end {c} -----")
print(results)
results.drop(columns = 'index').to_csv(dp['agen'], index = False)
