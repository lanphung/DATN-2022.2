...
def write_to_doc(asia_gene, world_gene, cancer_name):
    ...
    data.to_csv(path+f"raw_output/pre_data/{cancer_name}_cancer/drug/panel_gene_asia.csv")
    ...
    data = data.sort_values('ratio', ascending=False).reset_index(drop=True)
    data.to_csv(path+f"raw_output/pre_data/{cancer_name}_cancer/drug/panel_gene_world.csv")
    for k in range(0, min(len(data), listing_num)):
        para += f"{k}, {data['list'][k]} ({round(data['ratio'][k]*100,4)}%)\n"
    mydoc.add_paragraph(para)
    mydoc.save(path+f"output/panels/{cancer_name}_cancer_gene_list.docx")
...
def _chart_create(ratio, ratio_census, cancer_name, flag, title0, title1):
    ...
    if flag == 'g':
        filename = path+f"raw_output/pre_data/{cancer_name}_cancer/charts/{cancer_name}_cancer"
    ...
    x = -0.04,fontsize = 12, labelpad=6)
        ...
        filename = path+f"raw_output/pre_data/{cancer_name}_cancer/charts/(variant){cancer_name}_gene"
